...
export class AuthEffects {
  ...
  private authPersistence = inject(AuthPersistenceService);
    ...
    this.actions$.pipe(
      ...
      mergeMap(({ user }) =>
        this.authService.updateUserProfile(user).pipe(
          map((updatedUser) =>
            AuthActions.updateUserProfileSuccess({ user: updatedUser }),
          ),
          catchError((error) =>
              ...
              if (persistedAuth) {
                ...
                return this.authService.validateToken(persistedAuth.token).pipe(
                  switchMap((isValid) => {
                    if (isValid) {
                      return of(
                        AuthActions.initializeAuthState({
    ...
    this.actions$.pipe(
      ...
      switchMap((action) =>
        this.authService.resetPassword(action.email).pipe(
          ...
          catchError((error) =>
            of(AuthActions.sendResetPasswordEmailFailure({ error })),
          ),
        ),
      ),
    ),
    ...
    return from(this.supabaseService.getUserProfile(authUser.id)).pipe(
      map((profile: any) => {
        ...
        return {
          ...
          fullName:
            profile?.full_name ||
            `${profile?.first_name || registerRequest?.firstName || ''} ${profile?.last_name || registerRequest?.lastName || ''}`.trim(),
  ...
  private createUserFromAuthResponse(
    authUser: AuthUser | null,
    email: string,
  ): User {
    if (!authUser) {
  ...
  private createUserFromRegisterResponse(
    authUser: AuthUser | null,
    registerRequest: any,
  ): User {
    if (!authUser) {
    ...
    return {
      ...
      fullName:
        `${registerRequest.firstName || ''} ${registerRequest.lastName || ''}`.trim(),
