    ...
    <app-admin-form
      ...
      <div class="mt-8">
        ...
        <textarea
          ...
          rows="4"
          class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
          placeholder="Add any additional notes about this order..."></textarea>
      </div>
    </app-admin-form>
  `
})
export class OrderFormComponent implements OnInit {
  ...
  private async loadOrder(): Promise<void> {
    ...
    this.loading = true;
    try {
      const order = await this.supabaseService.getTableById('orders', this.orderId);
      if (order) {
        // Convert date to local datetime format for input
        if (order.order_date) {
          const date = new Date(order.order_date);
          const localDateTime = new Date(date.getTime() - date.getTimezoneOffset() * 60000).toISOString().slice(0, 16);
          order.order_date = localDateTime;
        }
