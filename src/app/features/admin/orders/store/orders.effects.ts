...
export class OrdersEffects {
  ...
  loadUserOrders$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap(({ userEmail }) =>
        this.supabaseService
          .getTable('orders', { customer_email: userEmail })
          .then(async (ordersData) => {
            console.log(
              'Loading orders for user email:',
              userEmail,
              'Found orders:',
              ordersData?.length || 0,
            );
            ...
            for (const orderData of ordersData || []) {
              try {
                ...
                console.error(
                  'Error loading order items for user order:',
                  itemError,
                );
                // Continue with order but empty items
                const order: Order = {
            ...
            console.log(
              'User orders loaded successfully:',
              ordersWithItems.length,
            );
            return OrdersActions.loadUserOrdersSuccess({
              orders: ordersWithItems,
            });
          })
          .catch((error) => {
            console.error('Error loading user orders:', error);
            return OrdersActions.loadUserOrdersFailure({
              error: error.message,
            });
          }),
      ),
  ...
  updateOrderStatus$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap(({ orderId, status }) =>
        this.supabaseService
          .adminUpdateRecord('orders', orderId, { status: status as any })
          .then(async () => {
            // Handle stock restoration for cancelled orders
            if (status === 'cancelled') {
              try {
                console.log(
                  'Order cancelled, restoring stock for order:',
                  orderId,
                );
                // Get order items to restore stock
                const orderItems = await this.supabaseService.getTable(
                  'order_items',
                  { order_id: orderId },
                );
                ...
                console.error(
                  'Error restoring stock for cancelled order:',
                  error,
                );
    ...
    () =>
      this.actions$.pipe(
        ...
        switchMap(async ({ orderId, status }) => {
          try {
            console.log(
              'Sending order status change emails for order:',
              orderId,
              'new status:',
              status,
            );
            ...
            const orderData = await this.supabaseService.getTable('orders', {
              id: orderId,
            });
            const order =
            ...
            const customerEmailSent =
              await this.emailService.sendOrderStatusChangeEmail(emailData);
            console.log(
              'Customer order status change email sent:',
              customerEmailSent,
            );

            // Send email to admin
            const adminEmailSent =
              await this.emailService.sendOrderStatusChangeNotificationToAdmin(
                emailData,
              );
            console.log(
              'Admin order status change notification email sent:',
              adminEmailSent,
            );
  ...
  updatePaymentStatus$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap(({ orderId, paymentStatus }) =>
        this.supabaseService
          .adminUpdateRecord('orders', orderId, {
            payment_status: paymentStatus as any,
          })
          .then(() =>
            OrdersActions.updatePaymentStatusSuccess({
              orderId,
              paymentStatus,
            }),
          )
    ...
    () =>
      this.actions$.pipe(
        ...
        switchMap(async ({ orderId, paymentStatus }) => {
          try {
            console.log(
              'Sending payment status change emails for order:',
              orderId,
              'new payment status:',
              paymentStatus,
            );
            ...
            if (!order) {
              console.error(
                'Order not found for payment status change email:',
                orderId,
              );
              return { success: false, error: 'Order not found' };
            }
            ...
            const emailData = {
              ...
              customerEmail: order.customer_email,
              newStatus:
                croatianPaymentStatusTranslations[paymentStatus] ||
                paymentStatus,
            };
            ...
            const customerEmailSent =
              await this.emailService.sendOrderStatusChangeEmail(emailData);
            console.log(
              'Customer payment status change email sent:',
              customerEmailSent,
            );
            ...
            console.log(
              'Admin payment status change notification email sent:',
              adminEmailSent,
            );
  ...
  confirmPurchase$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap(({ orderId }) =>
        this.supabaseService
          .adminUpdateRecord('orders', orderId, { payment_status: 'paid' })
          .then(() => OrdersActions.confirmPurchaseSuccess({ orderId }))
          .catch((error) =>
  ...
  deleteOrder$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap(({ orderId }) =>
        ...
        this.supabaseService
          .getTable('order_items', { order_id: orderId })
          .then(async (orderItems) => {
            // Delete all order items first
            if (orderItems && orderItems.length > 0) {
              for (const item of orderItems) {
                await this.supabaseService.adminDeleteRecord(
                  'order_items',
                  item.id,
                );
              }
  ...
  createB2COrder$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap(({ orderData, cartItems }) => {
        ...
        return this.createOrderWithStockManagement(orderData, cartItems)
          .then((result) => {
            console.log('B2C Order creation successful in effect:', result);
            return OrdersActions.createB2COrderSuccess({
              order: result.order,
          ...
          .catch((error: any) => {
            ...
            return OrdersActions.createB2COrderFailure({
              error: error.message,
            });
          });
      }),
  ...
  private async createOrderWithStockManagement(
    orderData: any,
    cartItems: any[],
  ): Promise<{ order: Order; orderNumber: string }> {
    try {
      ...
      const stockAdjustmentSuccess =
        await this.supabaseService.processOrderStockAdjustment(cartItems, true);
      ...
      if (orderData.user_id === null) {
        ...
        console.log(
          'Creating authenticated user order with user_id:',
          orderData.user_id,
        );
        orderRecord = await this.supabaseService.createRecord(
          'orders',
          orderData,
        );
      }
      ...
      for (const item of cartItems) {
        const orderItemData = {
          ...
          product_id:
            item.productId === '00000000-0000-0000-0000-000000000001'
              ? null
              : item.productId,
    ...
    () =>
      this.actions$.pipe(
        ...
        switchMap(async ({ order, orderNumber }) => {
          try {
            console.log(
              'Sending order confirmation emails for order:',
              orderNumber,
            );
  ...
  loadUserReviews$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap(({ userId }) =>
        this.supabaseService
          .getTable('reviews', { user_id: userId })
          .then(async (reviewsData) => {
            console.log(
              'Loading reviews for user ID:',
              userId,
              'Found reviews:',
            ...
            for (const reviewData of reviewsData || []) {
              try {
                ...
                const productData = await this.supabaseService.getTable(
                  'products',
                  {
                ...
                console.error(
                  'Error loading product for user review:',
                  productError,
                );
            ...
            console.log(
              'User reviews loaded successfully:',
              reviewsWithProducts.length,
            );
            return OrdersActions.loadUserReviewsSuccess({
              reviews: reviewsWithProducts,
            });
          })
          .catch((error) => {
            console.error('Error loading user reviews:', error);
            return OrdersActions.loadUserReviewsFailure({
              error: error.message,
            });
          }),
      ),
