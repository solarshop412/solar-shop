...
export class OfferFormComponent implements OnInit {
  ...
  private initForm(): void {
    ...
    this.offerForm = this.fb.group({
      ...
      start_date: [
        this.formatDateTimeLocal(defaultStart),
        [Validators.required],
      ],
  ...
  private setupDiscountChangeListeners(): void {
    ...
    this.offerForm
      .get('discount_type')
      ?.valueChanges.subscribe((discountType) => {
  ...
  private onDiscountTypeChange(discountType: string): void {
    ...
    const discountValue = Number(
      this.offerForm.get('discount_value')?.value || 0,
    );
  ...
  private applyDiscountTypeToAllProducts(
    discountType: string,
    ...
    if (discountType === 'percentage') {
      ...
      this.productsArray.controls.forEach((productControl) => {
        ...
        productControl
          .get('discount_type')
          ?.setValue('fixed_amount', { emitEvent: false });
      });
    }
  }
  ...
  private checkEditMode(): void {
    ...
    this.title.setTitle(
      this.isEditMode
        ? 'Edit Offer - Solar Shop Admin'
        : 'Create Offer - Solar Shop Admin',
    );
  }
      ...
      if (data) {
        ...
        const formData = {
          ...
          start_date: data.start_date
            ? this.formatDateTimeLocal(new Date(data.start_date))
            : '',
          end_date: data.end_date
            ? this.formatDateTimeLocal(new Date(data.end_date))
            : '',
        };
  ...
  async onSubmit(formValue: any): Promise<void> {
    ...
    try {
      ...
      const applicableCategoryIds =
        applyToCategory && categoryId ? [categoryId] : [];
      ...
      const finalOfferData = {
        ...
        start_date: offerData.start_date
          ? new Date(offerData.start_date).toISOString()
          : null,
        end_date: offerData.end_date
          ? new Date(offerData.end_date).toISOString()
          : null,
        max_usage:
          offerData.max_usage === '' ||
          offerData.max_usage === null ||
          offerData.max_usage === undefined
            ? null
            : Number(offerData.max_usage),
  ...
  private async saveOfferProducts(
    offerId: string,
    products: any[],
  ): Promise<void> {
    try {
      ...
      if (products.length > 0) {
        const offerProducts = products.map((product, index) => {
          ...
          return {
            ...
            discounted_price: this.calculateDiscountedPriceByType(
              product.price,
              discountValue,
              discountType,
            ),
  ...
  async onCategoryChange(categoryId: string): Promise<void> {
    ...
    if (existingProducts.length > 0) {
      ...
      const promptMessage = this.translationService.translate(
        'admin.offersForm.categoryChangePrompt',
        {
  ...
  applyDiscountToCategory(): void {
    ...
    const productsToAdd = this.categoryProducts.filter(
      (product) => !alreadyAddedIds.includes(product.id),
    );

    if (productsToAdd.length === 0) {
      alert(
        this.translationService.translate(
          'admin.offersForm.allCategoryProductsAdded',
        ),
      );
      return;
    }
  ...
  private createProductFormGroup(product?: any): FormGroup {
    ...
    const globalDiscountValue = Number(
      this.offerForm.get('discount_value')?.value || 0,
    );
  ...
  addProduct(): void {
    ...
    if (selectedCategoryId) {
      ...
      const categoryProducts = this.allProducts.filter(
      ...
      const availableProducts = categoryProducts.filter(
        (product) => !alreadyAddedProducts.includes(product.id),
      );
  ...
  addProductToOffer(product: any): void {
    ...
    if (existingIndex === -1) {
      ...
      if (selectedCategoryId && product.category_id !== selectedCategoryId) {
        alert(
          this.translationService.translate(
            'admin.offersForm.productNotInSelectedCategory',
          ),
        );
        return;
      }
      ...
      alert(
        this.translationService.translate(
          'admin.offersForm.productAlreadyAdded',
        ),
      );
    }
  }
  ...
  onProductSelect(productId: string, index: number): void {
    ...
    if (product) {
      ...
      if (globalDiscountType === 'percentage' && globalDiscountValue > 0) {
        ...
        globalDiscountType === 'fixed_amount' &&
  ...
  setProductDiscountType(index: number, type: string): void {
    ...
    if (globalDiscountType && type !== globalDiscountType) {
      alert(
        `Product discounts must match the global discount type: ${globalDiscountType}`,
      );
      return;
    }
  ...
  getTotalDiscountedPrice(): number {
    const totalDiscounted = this.productsArray.controls.reduce(
      (total, productControl) => {
        const price = Number(productControl.get('price')?.value || 0);
        const discountType =
          productControl.get('discount_type')?.value || 'percentage';
        const discountValue =
          discountType === 'percentage'
            ? Number(productControl.get('discount_percentage')?.value || 0)
            : Number(productControl.get('discount_amount')?.value || 0);
        const discountedPrice = this.calculateDiscountedPriceByType(
          price,
          discountValue,
          discountType,
        );
  ...
  validateProductDiscountLimits(index: number): void {
    ...
    if (globalDiscountType === 'percentage') {
      ...
      const currentProductDiscount = Number(
        product.get('discount_percentage')?.value || 0,
      );

      if (currentProductDiscount !== globalDiscountValue) {
        product.get('discount_percentage')?.setValue(globalDiscountValue);
        const message = this.translationService.translate(
          'admin.offersForm.productDiscountMustMatch',
          { value: globalDiscountValue, type: '%' },
        );
        alert(message);
      }
    } else if (globalDiscountType === 'fixed_amount') {
      ...
      const currentProductDiscount = Number(
        product.get('discount_amount')?.value || 0,
      );
      ...
      if (currentProductDiscount !== globalDiscountValue) {
        ...
        const message = this.translationService.translate(
          'admin.offersForm.productDiscountMustMatch',
          { value: globalDiscountValue.toFixed(2), type: '€' },
        );
        alert(message);
      }
    }
  ...
  getTotalProductDiscountsExcludingIndex(
    excludeIndex: number,
