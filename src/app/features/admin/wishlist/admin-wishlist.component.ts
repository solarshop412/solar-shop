...
export class AdminWishlistComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.title.setTitle(
      this.translationService.translate('adminWishlist.title') +
  ...
  private async loadUserWishlists(): Promise<void> {
    ...
    try {
      ...
      const { data: wishlistData, error: wishlistError } =
        await this.supabaseService.client.from('wishlist').select('*');
  ...
  private async viewWishlistDetails(
    userSummary: UserWishlistSummary,
  ): Promise<void> {
    ...
    try {
      ...
      for (const wishlistItem of wishlistData) {
        try {
          ...
          console.warn(
            'Could not load product details for:',
  ...
  private async deleteUserWishlist(
    userSummary: UserWishlistSummary,
  ): Promise<void> {
    if (
      !confirm(
        `Are you sure you want to delete all wishlist items for ${userSummary.userName}? This action cannot be undone.`,
      )
    ) {
      return;
    }
