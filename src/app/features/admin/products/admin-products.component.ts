...
export class AdminProductsComponent implements OnInit {
    ...
    try {
      ...
      for (const row of csvData) {
        try {
          ...
          if (categoryName) {
            const category = await this.supabaseService.getTable('categories', {
              name: categoryName,
            });
            if (category.length === 0) {
              const categorySlug = categoryName
                ...
                .replace(/[^a-z0-9]+/g, '-')
                .replace(/(^-|-$)/g, '');
              const newCategory = await this.supabaseService.createRecord(
                'categories',
                {
                  name: categoryName,
                  slug: categorySlug,
          ...
          const productData = {
            ...
            stock_quantity: parseInt(
              row.stock_quantity || row['Stock Quantity'] || '0',
            ),
            stock_status:
              parseInt(row.stock_quantity || row['Stock Quantity'] || '0') > 0
                ? ('in_stock' as const)
                : ('out_of_stock' as const),
            is_active:
              (row.is_active || row['Is Active'] || 'true').toLowerCase() ===
              'true',
            is_featured:
              (
                row.is_featured ||
            ...
            is_on_sale:
              (row.is_on_sale || row['Is On Sale'] || 'false').toLowerCase() ===
              'true',
            tags: row.tags
            ...
            certifications: row.certifications
              ...
              : [],
            estimated_delivery_days: parseInt(
              row.estimated_delivery_days || '7',
            ),
            installation_required:
              (row.installation_required || 'false').toLowerCase() === 'true',
            free_shipping:
              (row.free_shipping || 'false').toLowerCase() === 'true',
            rating_average: 0,
            rating_count: 0,
          ...
          const existingProduct = await this.supabaseService.getTable(
            'products',
            { sku: productData.sku },
          );
          ...
          const newProduct = await this.supabaseService.createRecord(
            'products',
            productData,
          );
          if (newProduct) {
          ...
          errors.push(
            `Error creating product: ${JSON.stringify(row)} - ${errorMessage}`,
          );
          errorCount++;
        }
      }
      ...
      for (const row of csvData) {
        try {
          ...
          if (
            !productSku ||
            !relationshipType ||
            (!relatedProductSku && !relatedCategoryName)
          ) {
            continue;
          }
          ...
          if (relatedProductSku) {
            ...
            if (!relatedProductId) {
              ...
              const existingRelatedProduct =
                await this.supabaseService.getTable('products', {
                  sku: relatedProductSku,
                });
          ...
          if (relatedCategoryName) {
            const relatedCategory = await this.supabaseService.getTable(
              'categories',
          ...
          if (relatedProductId || relatedCategoryId) {
            const relationshipData = {
              ...
              is_active:
                (row.relationship_active || 'true').toLowerCase() === 'true',
            };
          ...
          const errorMessage =
            error instanceof Error ? error.message : 'Unknown error';
          errors.push(
            `Error creating relationship: ${JSON.stringify(row)} - ${errorMessage}`,
          );
        }
      }
  ...
  private async loadProducts(): Promise<void> {
    ...
    try {
      ...
      console.warn(
        'Products table not found in database. Using mock data as placeholder.',
      );
    } finally {
      this.loadingSubject.next(false);
    }
   ...
   */
  async onDisplayOrderChanged(event: {
    item: any;
    column: string;
    value: number;
  }): Promise<void> {
    ...
    try {
      ...
      const conflictingProduct = products.find(
        (p) => p.display_order === newDisplayOrder && p.id !== item.id,
      );

      if (conflictingProduct) {
        // Shift all products at or above the new position down by 1
        const productsToShift = products.filter(
          (p) => p.id !== item.id && p.display_order >= newDisplayOrder,
        );
  ...
  private checkAndRunAutomaticSync(): void {
    ...
    if (!lastSync) {
      ...
      console.log(
        '[Admin Products] No previous sync found, running automatic sync...',
      );
    ...
    if (hoursSinceLastSync >= 24) {
      console.log(
        '[Admin Products] Last sync was more than 24 hours ago, running automatic sync...',
      );
      this.autoUpdateErpStock();
    } else {
      console.log(
        `[Admin Products] Last sync was ${Math.round(hoursSinceLastSync)} hours ago, skipping automatic sync.`,
      );
    }
  }
  ...
  private async autoUpdateErpStock(): Promise<void> {
    ...
    try {
      ...
      console.log(
        `[Admin Products] Updating stock for ${skus.length} products...`,
      );
      ...
      for (let i = 0; i < skus.length; i += batchSize) {
        ...
        console.log(
          `[Admin Products] Processing batch ${currentBatch} of ${totalBatches} (${batch.length} SKUs)`,
        );
        ...
        const batchRequests = batch.map((sku) =>
          this.erpService.getStockBySku(sku).pipe(
            catchError((err) => {
              console.error(
                `[Admin Products] Error fetching stock for ${sku}:`,
                err,
              );
        ...
        try {
          ...
          if (batchResults) {
            ...
            batch.forEach(() =>
              stockResponses.push({ success: false, error: 'No results' }),
            );
          ...
          batch.forEach(() =>
            stockResponses.push({ success: false, error: 'Batch failed' }),
          );
          processedCount += batch.length;
        }
        ...
        if (i + batchSize < skus.length) {
          console.log(
            `[Admin Products] Waiting ${delayBetweenBatches}ms before next batch...`,
          );
      ...
      for (const product of products) {
        ...
        if (
          ...
          const updatePromise = this.supabaseService
            ...
            .catch((err) => {
              console.error(
                `[Admin Products] Failed to update DB for product ${product.id}:`,
                err,
              );
            });
          ...
          if (!stockResponse?.success) {
            console.log(
              `[Admin Products] SKU ${product.sku} not found in ERP, skipping DB update`,
            );
          }
          updatedProducts.push(product);
        }
      ...
      console.log(
        '[Admin Products] ERP stock update completed successfully (memory + database)',
      );
  ...
  private async deleteProduct(product: any): Promise<void> {
    try {
      ...
      this.showSuccess(
        ...
        this.translationService.translate('admin.productDeletedSuccessfully'),
      );
      this.loadProducts();
    } catch (error) {
      ...
      this.showSuccess(
        this.translationService.translate('common.error'),
        this.translationService.translate('admin.productDeletedError'),
      );
    }
  }
