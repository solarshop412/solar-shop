...
export class CompanyPricingEffects {
    ...
    this.actions$.pipe(
      ...
      switchMap(() => {
        ...
        return this.supabaseService.client
          ...
          .eq('status', 'approved')
          .then(({ data, error }) => {
            console.log('CompanyPricingEffects: Supabase query result:', {
              data,
              error,
            });
            if (error) {
      ...
      catchError((error) => {
        ...
        return of(
          CompanyPricingActions.loadCompaniesFailure({ error: error.message }),
        );
      }),
    ),
  );
    ...
    this.actions$.pipe(
      ...
      switchMap(async () => {
        try {
          ...
          const { data: productCategories, error: pcError } =
          ...
          return CompanyPricingActions.loadProductsFailure({
            error: error.message,
          });
        }
      }),
      catchError((error) =>
        of(CompanyPricingActions.loadProductsFailure({ error: error.message })),
      ),
    ),
  );
    ...
    this.actions$.pipe(
      ...
      switchMap(() =>
        this.supabaseService
          .getTable('company_pricing')
          .then((data) => {
            const companyPricing = data || [];
            return CompanyPricingActions.loadCompanyPricingSuccess({
              companyPricing,
            });
          })
          .catch((error) =>
            CompanyPricingActions.loadCompanyPricingFailure({
              error: error.message,
            }),
          ),
      ),
    ...
    this.actions$.pipe(
      ...
      switchMap((action) => {
        return this.supabaseService
          .createRecord('company_pricing', action.pricing)
          .then((data) => {
            if (!data) {
              throw new Error('No data returned from create operation');
            }
            return CompanyPricingActions.createCompanyPricingSuccess({
              pricing: data as CompanyPricingActions.CompanyPricing,
            });
          });
      }),
      catchError((error) =>
        of(
          CompanyPricingActions.createCompanyPricingFailure({
            error: error.message,
          }),
        ),
      ),
  ...
  updateCompanyPricing$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap((action) => {
        return this.supabaseService
          .updateRecord('company_pricing', action.id, action.pricing)
          .then((data) => {
            if (!data) {
              throw new Error('No data returned from update operation');
            }
            return CompanyPricingActions.updateCompanyPricingSuccess({
              pricing: data as CompanyPricingActions.CompanyPricing,
            });
          });
      }),
      catchError((error) =>
        of(
          CompanyPricingActions.updateCompanyPricingFailure({
            error: error.message,
          }),
        ),
      ),
  ...
  deleteCompanyPricing$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap((action) => {
        return this.supabaseService
          .deleteRecord('company_pricing', action.id)
          .then(() => {
            return CompanyPricingActions.deleteCompanyPricingSuccess({
              id: action.id,
            });
          })
          .catch((error) =>
            CompanyPricingActions.deleteCompanyPricingFailure({
              error: error.message,
            }),
          );
      }),
