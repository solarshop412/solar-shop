...
export class AdminCategoriesComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.title.setTitle(
      this.translationService.translate('admin.categoriesForm.longTitle'),
    );
    this.loadCategories();
  }
  ...
  async onCsvImported(csvData: any[]): Promise<void> {
    if (!csvData || csvData.length === 0) {
      alert(
        this.translationService.translate(
          'admin.categoriesForm.noDataFoundInCsvFile',
        ),
      );
      return;
    }
    ...
    try {
      ...
      const categories = csvData.map((row) => ({
        ...
        slug: (row.slug || row.Slug || row.name || row.Name || '')
          .toLowerCase()
          .replace(/\s+/g, '-'),
      ...
      for (const category of categories) {
        ...
        const existingCategory = await this.supabaseService.getTable(
          'categories',
          { name: category.name },
        );
        if (existingCategory.length > 0) {
          alert(
            this.translationService.translate(
              'admin.categoriesForm.categoryAlreadyExists',
              { name: category.name },
            ),
          );
          continue;
      ...
      alert(
        this.translationService.translate(
          'admin.categoriesForm.successImportingCategories',
      ...
      alert(
        this.translationService.translate(
          'admin.categoriesForm.errorImportingCategories',
        ),
      );
    } finally {
  ...
  private async loadCategories(): Promise<void> {
    ...
    try {
      ...
      console.warn(
        'Categories table not found in database. Using mock data as placeholder.',
      );
    } finally {
      this.loadingSubject.next(false);
    }
  ...
  private async deleteCategory(category: any): Promise<void> {
    try {
      ...
      this.showSuccess(
        ...
        this.translationService.translate(
          'admin.categoriesForm.categoryDeletedSuccessfully',
        ),
      );
      this.loadCategories();
    } catch (error: any) {
      ...
      if (
        ...
        this.showSuccess(
          ...
          this.translationService.translate(
            'admin.categoriesForm.categoryHasRelatedProducts',
          ),
        );
      } else {
        this.showSuccess(
          this.translationService.translate('admin.categoriesForm.error'),
          this.translationService.translate(
            'admin.categoriesForm.errorDeletingCategory',
          ),
        );
      }
    }
