...
@Component({
    ...
    >
      ...
      <div>
        ...
        ></textarea>
        <div *ngIf="blogForm.get('summary')?.invalid && blogForm.get('summary')?.touched" class="mt-1 text-sm text-red-600">
          Summary is required
        </div>
      </div>
...
export class BlogFormComponent implements OnInit {
    ...
    private async loadAuthors(): Promise<void> {
        try {
            this.authors = await this.supabaseService.getTable('profiles');
        } catch (error) {
    ...
    onTitleChange(event: any): void {
        ...
        if (!this.blogForm.get('meta_title')?.value) {
            this.blogForm.patchValue({ meta_title: title });
        }
    }
