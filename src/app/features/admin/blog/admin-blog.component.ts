  ...
  tableConfig: TableConfig = {
    columns: [
      ...
      },
      {
        ...
        format: (value) => {
          return (
            this.translationService.translate(`admin.blogForm.${value}`) ||
            value
          );
        },
        width: '15%',
  ...
  async onCsvImported(csvData: any[]): Promise<void> {
    ...
    try {
      ...
      const blogPosts = csvData.map((row) => ({
        ...
        slug: (row.slug || row.Slug || row.title || row.Title || '')
          .toLowerCase()
          .replace(/\s+/g, '-'),
        content: row.content || row.Content || '',
        excerpt: row.excerpt || row.Excerpt || '',
        featured_image_url:
          row.featured_image_url || row['Featured Image URL'] || undefined,
        category_id: row.category_id || row['Category ID'] || undefined,
        tags: row.tags ? row.tags.split(',').map((t: string) => t.trim()) : [],
        status: (row.status || row.Status || 'draft') as
          | 'draft'
          | 'published'
          | 'archived',
        ...
        seo_description:
          row.seo_description || row['SEO Description'] || undefined,
        seo_keywords: row.seo_keywords
          ? row.seo_keywords.split(',').map((k: string) => k.trim())
          : [],
        reading_time: row.reading_time ? parseInt(row.reading_time) : undefined,
        is_featured:
          (row.is_featured || row['Is Featured'] || 'false').toLowerCase() ===
          'true',
      }));
