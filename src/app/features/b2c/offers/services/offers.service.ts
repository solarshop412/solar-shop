...
export class OffersService {
  ...
  private async fetchOffersFromSupabase(
    ...
    try {
      ...
      if (filters?.featured !== undefined) {
        filteredOffers = filteredOffers.filter(
          (offer) => offer.featured === filters.featured,
        );
      }
  ...
  private async fetchFeaturedOffers(limit?: number): Promise<Offer[]> {
    try {
      ...
      let offers = supabaseOffers.filter(
        (offer) => !offer.is_b2b && offer.featured,
      ); // Only B2C featured offers
      if (limit) {
        offers = offers.slice(0, limit);
      }
  ...
  private async convertSupabaseOffersToLocal(offers: any[]): Promise<Offer[]> {
    const offerPromises = offers.map((offer) =>
      this.convertSupabaseOfferToLocal(offer),
    );
  ...
  private async convertSupabaseOfferToLocal(offer: any): Promise<Offer | null> {
    try {
      ...
      const { originalPrice, discountedPrice } =
        await this.calculateOfferPricing(offer);
  ...
  private async calculateOfferPricing(
    offer: any,
  ): Promise<{ originalPrice: number; discountedPrice: number }> {
    try {
      ...
      if (error || !offerProducts || offerProducts.length === 0) {
        ...
        console.log(
          'calculateOfferPricing - No products, using placeholder logic:',
          {
            offerId: offer.id,
            discount_type: offer.discount_type,
            discount_value: offer.discount_value,
        ...
        if (offer.discount_type === 'percentage') {
          ...
          console.log(
            'Applied percentage discount to placeholder:',
            discountedPrice,
          );
        } else if (offer.discount_type === 'fixed_amount') {
          discountedPrice = Math.max(0, originalPrice - offer.discount_value);
          console.log(
            'Applied fixed amount discount to placeholder:',
            discountedPrice,
          );
        } else {
          console.log(
            'NO DISCOUNT APPLIED to placeholder - discount_type not recognized:',
            offer.discount_type,
          );
        }
      ...
      for (const offerProduct of offerProducts) {
        ...
        if (hasIndividualDiscount) {
          ...
          if (
            offerProduct.discount_type === 'fixed_amount' &&
            offerProduct.discount_amount > 0
          ) {
            totalDiscountAmount += offerProduct.discount_amount;
            console.log(
              `Product ${offerProduct.product_id} has individual fixed discount: €${offerProduct.discount_amount}`,
            );
          } else if (
            ...
            const productDiscount =
              productPrice * (offerProduct.discount_percentage / 100);
            totalDiscountAmount += productDiscount;
            console.log(
              `Product ${offerProduct.product_id} has individual percentage discount: ${offerProduct.discount_percentage}% = €${productDiscount}`,
            );
          }
        } else {
          ...
          if (offer.discount_type === 'percentage') {
            ...
            totalDiscountAmount += productDiscount;
            console.log(
              `Product ${offerProduct.product_id} uses general percentage discount: ${offer.discount_value}% = €${productDiscount}`,
            );
          } else if (offer.discount_type === 'fixed_amount') {
            ...
            const discountPerProduct =
              offer.discount_value / offerProducts.length;
            totalDiscountAmount += discountPerProduct;
            console.log(
              `Product ${offerProduct.product_id} uses general fixed discount: €${discountPerProduct} (€${offer.discount_value} / ${offerProducts.length} products)`,
            );
          }
        }
      }

      const totalDiscountedPrice = Math.max(
        0,
