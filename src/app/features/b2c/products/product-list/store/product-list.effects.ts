    ...
    this.actions$.pipe(
      ...
      switchMap(({ query }) =>
        this.productListService.getProductsWithPagination(query || {}).pipe(
          map((response) =>
            ProductListActions.loadProductsSuccess({ response }),
          ),
          catchError((error: any) =>
            of(
              ProductListActions.loadProductsFailure({
    ...
    this.actions$.pipe(
      ...
      switchMap(() =>
        this.productListService.getAllManufacturers().pipe(
          map((manufacturers) =>
            ProductListActions.loadAllManufacturersSuccess({ manufacturers }),
          ),
          catchError((error: any) =>
            of(
              ProductListActions.loadAllManufacturersFailure({
    ...
    this.actions$.pipe(
      ...
      switchMap(({ filters }) =>
        this.productListService.getCategoryCounts(filters).pipe(
          map((categoryCounts) =>
            ProductListActions.loadCategoryCountsSuccess({ categoryCounts }),
          ),
          catchError((error: any) =>
            of(
              ProductListActions.loadCategoryCountsFailure({
      ...
      switchMap(({ filters }) =>
        this.productListService.getManufacturerCounts(filters).pipe(
          map((manufacturerCounts) =>
            ProductListActions.loadManufacturerCountsSuccess({
              manufacturerCounts,
            }),
          ),
          catchError((error: any) =>
            of(
              ProductListActions.loadManufacturerCountsFailure({
