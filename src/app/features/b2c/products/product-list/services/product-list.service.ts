...
export class ProductListService {
  ...
  getCategoryCounts(
    filters?: CategoryCountFilters | Partial<ProductFilters>,
  ): Observable<{ [categoryName: string]: number }> {
  ...
  getManufacturerCounts(
    filters?: ManufacturerCountFilters | Partial<ProductFilters>,
  ): Observable<{ [manufacturerName: string]: number }> {
  ...
  private async fetchProductsWithPagination(
    query: ProductsQuery,
    ...
    try {
      ...
      if (
        query.priceRange &&
        (query.priceRange.min > 0 || query.priceRange.max > 0)
      ) {
      ...
      const sortCode = query.sortOption || '';
      ...
      if (
        ...
        supabaseQuery = supabaseQuery.order('display_order', {
          ascending: true,
        });
      }
  ...
  private async fetchProductsFromSupabase(
  ...
  private async searchProductsInSupabase(
    query: string,
    limit?: number,
  ): Promise<Product[]> {
    try {
  ...
  private async convertSupabaseProductsToLocal(
    products: any[],
  ): Promise<Product[]> {
  ...
  private async convertSupabaseProductToLocal(
    product: any,
    ...
    try {
      ...
      const primaryCategory =
        categoriesArray.find((cat) => cat.isPrimary) || categoriesArray[0];
      const legacyCategoryName = primaryCategory
  ...
  private async fetchCategoryCounts(
    ...
    try {
      ...
      const searchQuery =
        (filters as any)?.searchQuery || (filters as any)?.search;
      ...
      if (
        (filters as any)?.manufacturers &&
        (filters as any).manufacturers.length > 0
      ) {
  ...
  private async fetchManufacturerCounts(
    ...
    try {
      ...
      const { data: allManufacturersData, error: manufacturersError } =
        await this.supabaseService.client
          .from('products')
          .select('brand')
          .eq('is_active', true)
      ...
      if (
        ...
        (filters as any).categories.length > 0
      ) {
        // First, get the category IDs from category names
        const { data: categoryData } = await this.supabaseService.client
          .from('categories')
      ...
      data?.forEach((product: any) => {
        if (product.brand) {
          manufacturerCounts[product.brand] =
            (manufacturerCounts[product.brand] || 0) + 1;
        }
      });
  ...
  private getProductImage(images: any[]): string {
    if (images && images.length > 0) {
      ...
      return (
        primaryImage.url ||
        'assets/images/placeholders/solar-panels-default.jpg'
      );
    }
