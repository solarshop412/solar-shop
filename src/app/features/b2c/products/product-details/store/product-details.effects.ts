...
export class ProductDetailsEffects {
        ...
        this.productListService.getProductById(productId).pipe(
          map((product) => {
            if (product) {
              ...
              return ProductDetailsActions.loadProductFailure({
          ...
          catchError((error: any) =>
            of(
              ProductDetailsActions.loadProductFailure({
      ...
      switchMap(async ({ productId }) => {
        try {
          ...
          const filteredReviews = (reviews || []).filter((review: any) => {
            ...
            console.log(
              `Review ${review.id}: product_id=${review.product_id}, is_approved=${review.is_approved}, status=${review.status}, matchesProduct=${matchesProduct}, isApproved=${isApproved}, isNotHidden=${isNotHidden}`,
            );
            return matchesProduct && isApproved && isNotHidden;
          });

          const mappedReviews: ProductReview[] = filteredReviews
            .map((review: any) => {
          ...
          return ProductDetailsActions.loadProductReviewsSuccess({
  ...
  loadReviewsOnProductLoad$ = createEffect(() =>
    this.actions$.pipe(
      ...
      map(({ product }) =>
        ProductDetailsActions.loadProductReviews({ productId: product.id }),
      ),
    ),
  );
}
