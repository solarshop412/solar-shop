...
export class ProductsService {
  ...
  getProductsByCategories(
    ...
    return from(
      this.fetchProductsByCategories(categoryNames, excludeProductId, limit),
    ).pipe(
      catchError((error) => {
  ...
  private async fetchProductsByCategories(
    categoryNames: string[],
    excludeProductId?: string,
    limit: number = 10,
  ): Promise<Product[]> {
    try {
      ...
      if (productIdsFromJunction.length > 0) {
        ...
        query = query.or(
          `category_id.in.(${categoryIds.join(',')}),id.in.(${productIdsFromJunction.join(',')})`,
        );
      } else {
