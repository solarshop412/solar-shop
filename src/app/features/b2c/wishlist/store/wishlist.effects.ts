...
export class WishlistEffects {
  ...
  addToWishlist$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap(([action, user]) => {
        if (!user?.id) {
          return of(
            WishlistActions.addToWishlistFailure({
  ...
  removeFromWishlist$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap(([action, user]) => {
        if (!user?.id) {
          return of(
            WishlistActions.removeFromWishlistFailure({
              error: 'User not authenticated',
            }),
          );
        }
        ...
        return from(
          this.removeProductFromWishlist(user.id, action.productId),
        ).pipe(
          map(() =>
        ...
        tap(({ error }) => {
          const message = error.includes('already in wishlist')
            ? this.translationService.translate(
                'productDetails.alreadyInWishlist',
              )
            : this.translationService.translate(
                'productDetails.addToWishlistError',
              );
    ...
    () =>
      this.actions$.pipe(
        ...
        tap(() => {
          this.toastService.showSuccess(
            this.translationService.translate(
              'productDetails.removedFromWishlist',
            ),
          );
        }),
      ),
      ...
      this.actions$.pipe(
        ...
        tap(() => {
          this.toastService.showError(
            this.translationService.translate(
              'productDetails.removeFromWishlistError',
            ),
          );
        }),
      ),
    ...
    try {
      ...
      for (const wishlistItem of wishlistData) {
        try {
            ...
            const item: WishlistItem = {
              ...
              product: {
                ...
                images: product.images.map((img) => ({
                  ...
                  order: img.order,
                  type:
                    img.type === 'thumbnail' || img.type === 'technical'
                      ? 'gallery'
                      : (img.type as 'main' | 'gallery'),
                })),
          ...
          console.warn(
            'Failed to load product details for wishlist item:',
            productError,
          );
  ...
  private async addProductToWishlist(
    userId: string,
    productId: string,
  ): Promise<WishlistItem> {
    try {
      // Check if product is already in wishlist
      const { data: existingItems, error: checkError } =
        await this.supabaseService.client
          .from('wishlist')
          .select('*')
          .eq('user_id', userId)
      ...
      const { data: createdItems, error: insertError } =
        await this.supabaseService.client
          .from('wishlist')
          .insert(wishlistData)
          .select();
      ...
      if (productData && productData.length > 0) {
        ...
        if (productDetails.category_id) {
          const categoryData = await this.supabaseService.getTable(
            'categories',
            {
  ...
  private async removeProductFromWishlist(
    userId: string,
    productId: string,
  ): Promise<void> {
    try {
