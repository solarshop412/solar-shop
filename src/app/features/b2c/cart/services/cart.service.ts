...
export class CartService {
  ...
  applyCoupon(code: string): Observable<Cart> {
    ...
    return this.couponValidationService
      .validateCoupon(code, currentItems, this.currentUserId || undefined)
      .pipe(
        switchMap((validationResult: CouponValidationResult) => {
          ...
          ).pipe(map(() => this.createCartFromItems(this.getCartItemsArray())));
        }),
        catchError((error) => {
  ...
  async addToCartAsync(productId: string, quantity: number = 1): Promise<void> {
    try {
      ...
      const product = await this.supabaseService.getTableById(
        'products',
        productId,
      );
      ...
      if (existingItemIndex > -1) {
        ...
        const bundleItem = currentItems.find(
          (item) =>
            item.isBundle &&
        ...
        const cartItem: CartItem = {
          ...
          taxInfo: {
            ...
            taxClass: 'standard',
            taxRate: 0.1,
            taxAmount: actualSellingPrice * 0.1 * quantity, // Use actual selling price for tax
          },
        };
        ...
        // Sync to Supabase if authenticated
        if (this.isAuthenticated && this.currentUserId) {
          await this.supabaseService.addToCart(
            productId,
            quantity,
            actualSellingPrice,
            this.currentUserId,
  ...
  async removeFromCartAsync(itemId: string): Promise<void> {
    try {
      ...
      if (
        item.isBundle &&
        item.bundleProductIds &&
        item.bundleProductIds.length > 0
      ) {
        console.log(
          '🎁 Removing bundle item - checking if coupon needs to be removed from remaining items',
        );
        console.log(
          `   Bundle product IDs: ${item.bundleProductIds.join(', ')}`,
        );
        ...
        const bundleComplete =
          bundleItems.length === item.bundleProductIds.length;

        if (!bundleComplete) {
          console.log(
            '   ❌ Bundle is now incomplete - removing coupon from remaining items',
          );
          ...
          const bundleCoupon = currentCoupons.find(
            (c) => item.offerName && item.offerName.includes(c.code),
          );

          if (bundleCoupon) {
            console.log(`   Removing bundle coupon: ${bundleCoupon.code}`);
            this.appliedCoupons.next(
              currentCoupons.filter((c) => c.id !== bundleCoupon.id),
            );
            this.couponValidationService.removeCouponFromSession(
              bundleCoupon.code,
            );
          }
        } else {
          console.log(
            '   ✅ Bundle still complete - keeping coupon on remaining items',
          );
        }
      }
  ...
  private async loadCartFromSupabase(): Promise<void> {
    try {
      ...
      const supabaseCartItems = await this.supabaseService.getCartItems(
        this.currentUserId,
      );
  ...
  private async syncItemToSupabase(
    ...
    try {
      ...
      const existingItems = await this.supabaseService.getCartItems(
        this.currentUserId,
      );
  ...
  private async syncOfferItemToSupabase(
    ...
    try {
      ...
      const existingItem = existingItems.find(
        (item) => item.product_id === productId && item.offer_id === offerId,
      );
      ...
      if (existingItem) {
        ...
        await this.supabaseService.client.from('cart_items').insert({
  ...
  private async clearCartFromSupabase(): Promise<void> {
    try {
      ...
      const cartItems = await this.supabaseService.getCartItems(
        this.currentUserId,
      );
  ...
  private getTotalDiscountAmount(coupons: AppliedCoupon[]): number {
    ...
    const itemsWithIndividualDiscounts = cartItems.filter(
      (item) => item.offerSavings && item.offerSavings > 0,
    );

    // Check if there are any items with offer/bundle fields (even if savings is currently 0/undefined)
    const itemsWithOffers = cartItems.filter(
      (item) => item.offerId && item.offerDiscount !== undefined,
    );
    ...
    if (itemsWithIndividualDiscounts.length > 0) {
      ...
      console.log(
        ...
        itemsWithIndividualDiscounts.map((item) => ({
          name: item.name,
          price: item.price,
      ...
      console.log(
        'Total discount amount: 0.00 (already applied to item prices)',
      );
      return 0;
    }
    ...
    if (itemsWithOffers.length > 0) {
      console.log(
        '💰 DISCOUNT CALCULATION DEBUG (Offer items with no savings - incomplete bundle):',
      );
      console.log(
        'Items with offers but no savings:',
    ...
    console.log(
      'Applied coupons:',
      coupons.map((c) => ({ code: c.code, discountAmount: c.discountAmount })),
    );
  ...
  private calculateCartSummary(
    ...
    console.log(
      'Total calculation:',
      `${subtotal.toFixed(2)} - ${discount.toFixed(2)} = ${total.toFixed(2)}`,
    );
  ...
  private updateCartItems(items: CartItem[]): void {
    ...
    if (
      itemsWithUpdatedBundles.length === 0 &&
      this.appliedCoupons.value.length > 0
    ) {
  ...
  private updateCartSummary(items: CartItem[], discount?: number): void {
    ...
    const discountAmount =
      discount !== undefined
        ? discount
        : this.getTotalDiscountAmount(this.appliedCoupons.value);
  ...
  private async getCategoryName(categoryId: string): Promise<string> {
    try {
      const category = await this.supabaseService.getTableById(
        'categories',
        categoryId,
      );
  ...
  private async applyCouponAsync(
    ...
    try {
      ...
      const updatedItems = await this.applyIndividualCouponDiscounts(
        code,
        discountType,
      );
      ...
      if (updatedItems) {
        ...
        this.updateCartSummary(
          currentItems,
          this.getTotalDiscountAmount(allCoupons),
        );
      }
    } catch (error) {
  ...
  private async applyIndividualCouponDiscounts(
    couponCode: string,
    discountType: string,
    ...
    try {
      ...
      const { data: offerProducts, error: productsError } =
        await this.supabaseService.client
          .from('offer_products')
          .select('*')
      ...
      if (isBundle) {
        ...
        const bundleComplete =
          bundleItemsInCart.length === bundleProductIds.length;
        ...
        if (!bundleComplete) {
          console.log(
            '❌ Bundle is incomplete - not all products are in cart. Skipping discount application.',
          );
          const errorMessage =
            this.translationService.translate(
              'cart.bundleRequiresAllProducts',
            ) ||
      ...
      const updatedItems = currentItems.map((item) => {
        const productDiscount = offerProducts.find(
          (op) => op.product_id === item.productId,
        );

        console.log(
          `🔍 Checking item: ${item.name} (productId: ${item.productId})`,
        );
        console.log(
          `   Current state: offerId=${item.offerId}, offerSavings=${item.offerSavings}, isBundle=${item.isBundle}`,
        );
        console.log(`   Offer being applied: offerData.id=${offerData.id}`);
        console.log(
          `   Type comparison: item.offerId type=${typeof item.offerId}, offerData.id type=${typeof offerData.id}`,
        );
        ...
        if (item.offerId && String(item.offerId) === String(offerData.id)) {
          console.log(
            `⏭️ SKIP: ${item.name} already has this offer applied (ID: ${offerData.id})`,
          );
          return item;
        }
        ...
        if (productDiscount) {
          console.log(
            `✅ MATCH: Applying individual discount to ${item.name} (ID: ${item.productId})`,
          );
          console.log(
            `   Current price: €${item.price}, Original price: €${item.originalPrice}, Base price: €${item.basePrice}`,
          );
          ...
          const discountType =
            ...
            (productDiscount.discount_amount > 0
              ? 'fixed_amount'
              : productDiscount.discount_percentage > 0
                ? 'percentage'
                : 'unknown');
          ...
          console.log(
            `   Determined type: ${discountType}, amount: ${discountAmount}, percentage: ${discountPercentage}`,
          );
          ...
          if (discountType === 'fixed_amount' && discountAmount > 0) {
            ...
            console.log(
              `   💰 Fixed discount: €${offerDiscount} from CURRENT €${currentPrice} = €${discountedPrice}`,
            );
            ...
            console.log(
              `   📊 Percentage discount: ${offerDiscount}% from CURRENT €${currentPrice} = €${discountedPrice}`,
            );
          } else {
          ...
          console.log(
            `   ✅ Applied discount: CURRENT €${currentPrice} → €${discountedPrice} (saved €${actualSavings})`,
          );
          ...
          console.log(
            `❌ NO MATCH: ${item.name} (ID: ${item.productId}) - keeping original price`,
          );
        }
  ...
  private async removeCouponAsync(couponId: string): Promise<void> {
    try {
      ...
      this.updateCartSummary(
        currentItems,
        this.getTotalDiscountAmount(updatedCoupons),
      );
    } catch (error) {
  ...
  private async removeIndividualCouponDiscounts(
    couponCode: string,
  ): Promise<void> {
    try {
      console.log(
        'Removing individual product discounts for coupon:',
        couponCode,
      );
      ...
      const resetItems = currentItems.map((item) => {
        ...
        const hasCouponApplied =
          item.offerName &&
          item.offerName.includes(this.getCouponOfferName(couponCode));
        ...
        const restorePrice =
          item.offerOriginalPrice || item.basePrice || item.price;
      ...
      console.log(
        'Reset cart items after removing coupon discounts:',
        resetItems,
      );
  ...
  addAllToCartFromOffer(
    ...
    return from(
      this.addAllToCartFromOfferAsync(
        products,
        offerId,
        offerName,
        offerType,
  ...
  private async addToCartFromOfferAsync(
    ...
    try {
      ...
      const effectiveDiscount =
        individualDiscount !== undefined ? individualDiscount : offerDiscount;
      ...
      if (!isBundle) {
        ...
        console.log(
          'Bundle item - discount will be applied by recalculateBundleStatus',
        );
      }
      ...
      if (existingItemIndex > -1) {
        ...
        // Sync to Supabase if authenticated
        if (this.isAuthenticated && this.currentUserId) {
          await this.syncOfferItemToSupabase(
            productId,
            newQuantity,
            discountedPrice,
            offerId,
        ...
        const cartItem: CartItem = {
          ...
          availability: {
            ...
            estimatedDelivery: product.estimated_delivery_days
      ...
      console.log('Successfully added product to cart from offer:', {
        productId,
        offerId,
        offerName,
      });
  ...
  private async addAllToCartFromOfferAsync(
    ...
    for (const productData of products) {
      try {
        ...
        const discountToApply =
          productData.individualDiscount !== undefined
            ? productData.individualDiscount
            : offerDiscount;
        ...
        console.error(
          `Error adding product ${productData.productId} from offer:`,
          error,
        );
        skippedCount++;
      }
    ...
    console.log('addAllToCartFromOfferAsync - Completed:', {
      addedCount,
      skippedCount,
    });
  ...
  private recalculateBundleStatus(items: CartItem[]): CartItem[] {
    console.log(
      '🔄 B2C recalculateBundleStatus called with',
      items.length,
      'items',
    );
    ...
    const updatedItems = items.map((item) => {
      ...
      console.log('🔍 B2C Checking bundle for product:', item.name, {
        ...
        hasAll: item.bundleProductIds.every((id) =>
          productsInCart.includes(id),
        ),
      });

      // Check if bundle is complete
      const bundleComplete = item.bundleProductIds.every((id) =>
        productsInCart.includes(id),
      );
      ...
      const hasCouponApplied =
        item.offerSavings !== undefined &&
        item.offerSavings > 0 &&
        item.offerId;

      if (hasCouponApplied) {
        console.log(
          '✅ B2C Item already has coupon discount applied, skipping recalculation:',
          {
            product: item.name,
            offerSavings: item.offerSavings,
            price: item.price,
      ...
      if (bundleComplete && item.offerDiscount !== undefined) {
        ...
        savings = 0;
        console.log(
          '❌ B2C Bundle incomplete, removing discount for:',
          item.name,
        );
      }
    ...
    console.log(
      '✨ B2C recalculateBundleStatus complete, returning',
      updatedItems.length,
      'items',
    );
    return updatedItems;
  ...
  private async applyBundleDiscount(
    ...
    const bundleItemsInCart = currentItems.filter(
      (item) =>
      ...
      if (
        ...
        if (bundleComplete) {
          if (offerType === 'percentage') {
            ...
            const totalOriginalPrice = bundleItemsInCart.reduce(
              (sum, i) => sum + i.price,
              0,
            );
            const proportionalDiscount =
              (item.price / totalOriginalPrice) * offerDiscount;
