...
export class BlogService {
  ...
  private async fetchBlogPostsFromSupabase(
    filters?: BlogFilters,
  ): Promise<BlogPost[]> {
    try {
  ...
  private async fetchPostsByCategory(
    categoryId: string,
    limit?: number,
  ): Promise<BlogPost[]> {
    try {
  ...
  private async searchPostsInSupabase(
    query: string,
    limit?: number,
  ): Promise<BlogPost[]> {
    try {
  ...
  private convertSupabasePostsToLocal(posts: any[]): BlogPost[] {
    return posts
      .map((post) => this.convertSupabasePostToLocal(post))
