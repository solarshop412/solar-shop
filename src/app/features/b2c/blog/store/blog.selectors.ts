...
export const selectFilteredPosts = createSelector(
  ...
  (posts, filteredCategory, searchQuery) => {
    console.log('selectFilteredPosts called with:', {
      posts: posts.length,
      filteredCategory,
      searchQuery,
    });
    ...
    if (filteredCategory) {
      ...
      filteredPosts = filteredPosts.filter(
        (post) => post.category.id === filteredCategory,
      );
...
export const selectFeaturedPosts = createSelector(selectBlogPosts, (posts) =>
  posts.filter((post) => post.featured),
);

export const selectPostCategories = createSelector(selectBlogPosts, (posts) => {
