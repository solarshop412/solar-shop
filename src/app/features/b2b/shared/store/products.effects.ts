...
export class ProductsEffects {
  ...
  private getPrimaryImageUrl(images: any[]): string {
    ...
    return firstImage?.url || '/assets/images/product-placeholder.webp';
  }
  ...
  private async loadProductsWithPagination(
    query: ProductsActions.ProductsQuery,
  ): Promise<ProductsActions.ProductsResponse> {
    ...
    if (query.categories && query.categories.length > 0) {
      ...
      if (categoryData && categoryData.length > 0) {
        ...
        console.log(
          '🔗 B2B Product-category junction data:',
          productCategoryIds,
        );

        const productIdsFromJunction = (productCategoryIds || []).map(
          (pc) => pc.product_id,
        );
        console.log(
          '📦 B2B Product IDs from junction table:',
          productIdsFromJunction,
        );
        ...
        if (productIdsFromJunction.length > 0) {
          ...
          console.log(
            '📝 B2B Applying legacy category_id filter:',
            categoryIds,
          );
        ...
        console.log(
          '⚠️ B2B No matching categories found for:',
          query.categories,
        );
      }
    }
    ...
    if (
      selectedSort &&
      selectedSort.sortFields &&
      selectedSort.sortFields.length > 0
    ) {
      // Apply each sort field in order
      for (const sf of selectedSort.sortFields) {
        supabaseQuery = supabaseQuery.order(sf.field, {
          ascending: sf.direction === 'asc',
        });
      }
    } else {
    ...
    this.actions$.pipe(
      ...
      switchMap(({ companyId }) =>
        from(
          ...
          map(({ data, error }) => {
            if (error) {
              return ProductsActions.loadCompanyPricingFailure({
                error: error.message,
              });
            }
            ...
            return ProductsActions.loadCompanyPricingSuccess({
              pricing: mappedPricing,
            });
          }),
          catchError((error: any) =>
            of(
              ProductsActions.loadCompanyPricingFailure({
                error: error.message || 'Failed to load company pricing',
              }),
            ),
          ),
        ),
    ...
    this.actions$.pipe(
      ...
      switchMap(({ productId }) =>
        from(
          ...
          map(({ data, error }) => {
            if (error) {
              return ProductsActions.loadProductFailure({
                error: error.message,
              });
            }
    ...
    this.actions$.pipe(
      ...
      switchMap(() =>
        from(
          ...
          map(({ data, error }) => {
            if (error) {
              return ProductsActions.loadCategoriesFailure({
                error: error.message,
              });
            }
            return ProductsActions.loadCategoriesSuccess({
              categories: data || [],
            });
          }),
          catchError((error: any) =>
    ...
    this.actions$.pipe(
      ...
      switchMap(() =>
        from(this.fetchAllManufacturers()).pipe(
          map((manufacturers) =>
            ProductsActions.loadAllManufacturersSuccess({ manufacturers }),
          ),
          catchError((error: any) =>
            of(
              ProductsActions.loadAllManufacturersFailure({
    ...
    this.actions$.pipe(
      ...
      switchMap(({ filters }) =>
        from(this.fetchCategoryCounts(filters)).pipe(
          map((categoryCounts) =>
            ProductsActions.loadCategoryCountsSuccess({ categoryCounts }),
          ),
          catchError((error: any) =>
            of(
              ProductsActions.loadCategoryCountsFailure({
                error: error.message || 'Failed to load category counts',
              }),
            ),
          ),
        ),
    ...
    this.actions$.pipe(
      ...
      switchMap(({ filters }) =>
        from(this.fetchManufacturerCounts(filters)).pipe(
          map((manufacturerCounts) =>
            ProductsActions.loadManufacturerCountsSuccess({
              manufacturerCounts,
            }),
          ),
          catchError((error: any) =>
            of(
              ProductsActions.loadManufacturerCountsFailure({
                error: error.message || 'Failed to load manufacturer counts',
              }),
            ),
          ),
        ),
  ...
  private async fetchAllManufacturers(): Promise<string[]> {
    ...
    const uniqueManufacturers = [
      ...new Set((data || []).map((product) => product.brand).filter(Boolean)),
    ];
    return uniqueManufacturers.sort();
  }

  private async fetchCategoryCounts(
    filters?: ProductsActions.CategoryCountFilters,
    ...
    (data || []).forEach((product) => {
      ...
      if (
        product.category_id &&
        individualCategoryCounts[product.category_id]
      ) {
    ...
    const hierarchicalCounts = this.calculateHierarchicalCounts(
      categoryHierarchy,
  ...
  private calculateHierarchicalCounts(
    ...
    const collectDescendantProducts = (
      categoryId: string,
      ...
      if (individualCounts[categoryId]) {
        individualCounts[categoryId].forEach((productId) =>
      ...
      if (categoryInfo && categoryInfo.children.length > 0) {
        categoryInfo.children.forEach((childId) => {
          const childProducts = collectDescendantProducts(
            childId,
            new Set(visited),
          );
  ...
  private async fetchManufacturerCounts(
    filters?: ProductsActions.ManufacturerCountFilters,
  ): Promise<{ [manufacturerName: string]: number }> {
    ...
    (data || []).forEach((product) => {
      ...
      if (manufacturerName) {
        filteredCounts[manufacturerName] =
          (filteredCounts[manufacturerName] || 0) + 1;
      }
    });
      ...
      switchMap(({ sku }) => {
        ...
        return this.erpService.getStockBySku(sku).pipe(
          map((response) => {
            ...
            if (response.success) {
              const stockItems: ProductsActions.ErpStockItem[] = (
                response.data || []
              ).map((item) => ({
                sku: item.sku,
                unitId: item.unitId,
              ...
              console.log(
                `[ERP] Stock items loaded: ${stockItems.length} items`,
              );
              ...
              return ProductsActions.loadErpStockFailure({
                error:
                  response.error || 'Nije moguće učitati zalihe iz ERP sustava',
              });
            }
          }),
          catchError((error) => {
            ...
            return of(
              ProductsActions.loadErpStockFailure({
