...
export const selectProductsFeature =
  createFeatureSelector<ProductsState>('b2bProducts');
...
export const selectProductsWithPricing = createSelector(
  ...
  (products, companyPricing): ProductWithPricing[] => {
    return products.map((product) => {
      const customPricing = companyPricing.find(
      ...
      const company_minimum_order = customPricing
        ? customPricing.minimum_order
        : undefined;
      ...
      return {
        ...
        company_pricing_tiers:
          company_pricing_tiers.length > 0 ? company_pricing_tiers : undefined,
      };
    });
  },
);
...
export const selectProductById = (productId: string) =>
  createSelector(selectProductsWithPricing, (products: ProductWithPricing[]) =>
