...
export class B2bPaymentComponent implements OnInit, OnDestroy {
  ...
  async ngOnInit(): Promise<void> {
    ...
    combineLatest([
      ...
      .pipe(takeUntil(this.destroy$))
      .subscribe(async ([user, items, total]) => {
  ...
  async onSubmit(): Promise<void> {
    ...
    try {
      ...
      const shippingInfo = JSON.parse(
        localStorage.getItem('b2b_shipping_info') || '{}',
      );
      ...
      const orderData = {
        ...
        customer_name:
          `${this.currentUser.firstName} ${this.currentUser.lastName}`.substring(
            0,
            50,
          ), // Limit to 50 chars
        ...
        status: 'pending',
        payment_status:
          paymentData.paymentMethod === 'credit_30_days'
            ? 'pending'
            : 'pending',
        payment_method:
          paymentData.paymentMethod === 'payment_upon_collection'
            ? 'cash_on_delivery'
            : (paymentData.paymentMethod || '').substring(0, 20),
        shipping_address: {
          ...
          shippingMethod: (shippingInfo.shippingMethod || 'standard').substring(
            0,
            20,
          ),
          ...
          purchaseOrderNumber: (
            shippingInfo.purchaseOrderNumber || ''
          ).substring(0, 50),
        },
        billing_address: {
  ...
  private showSuccessToast(): void {
    ...
    const successMessage = this.translationService.translate(
      'b2bCheckout.orderPlacedSuccessfully',
    );
