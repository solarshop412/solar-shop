...
export class B2BCartService {
  ...
  loadCart(
    ...
    items: B2BCartItem[];
    companyName: string;
    appliedCoupons: B2BAppliedCoupon[];
  ...
  addToCart(
    ...
    return from(this.getProductWithPricing(productId, companyId)).pipe(
      map((product) => {
        ...
        const tierInfo = this.calculateTieredPrice(
          quantity,
          product,
          baseCompanyPrice,
        );
        ...
        if (
          options.partnerOfferId &&
          options.isBundle &&
          options.bundleProductIds
        ) {
          ...
          shouldApplyDiscount = options.bundleProductIds.every((id) =>
            uniqueProducts.includes(id),
          );
        }
        ...
        const standardSavingsPerUnit = Math.max(
          0,
          retailPrice - tierInfo.unitPrice,
        );
        const additionalSavingsPerUnit = Math.max(
          0,
          totalSavingsPerUnit - standardSavingsPerUnit,
        );
        ...
        const newItem: B2BCartItem = {
          ...
          partnerOfferOriginalPrice: options.partnerOfferId
            ? retailPrice
            : undefined,
          partnerOfferValidUntil: options.partnerOfferValidUntil,
          partnerOfferAppliedAt: options.partnerOfferId
            ? new Date()
            : undefined,
          ...
          bundleComplete:
            shouldApplyDiscount && options.isBundle ? true : false,
        };
        ...
        const parsedItems = updatedCart.map((item) =>
          this.parseStoredCartItem(item),
        );
        const tieredItems = this.recalculateTieredPricing(parsedItems);

        // Recalculate bundle completion and apply discounts if needed
        const bundleUpdatedItems = this.recalculateBundleStatus(
          tieredItems,
          companyId,
        );
        ...
        const updatedItem = bundleUpdatedItems.find(
   ...
   */
  updateCartItemWithPricing(
    productId: string,
    quantity: number,
    companyId: string,
  ): Observable<B2BCartItem | null> {
    const storageKey = this.STORAGE_KEY + companyId;
    const storedCart = localStorage.getItem(storageKey);
    let items: B2BCartItem[] = storedCart ? JSON.parse(storedCart) : [];
  ...
  applyCoupon(
    ...
    companyId: string,
  ): Observable<{ coupon: Coupon; discount: number }> {
    ...
    return new Observable<boolean>((observer) => {
      try {
        ...
        if (updatedCoupons.length === coupons.length) {
          throw new Error(
            this.translationService.translate('cart.couponNotFound'),
          );
        }
  ...
  private saveCartItem(companyId: string, newItem: B2BCartItem): void {
    ...
    if (existingIndex >= 0) {
      ...
      const companyPrice =
        pricingSource.companyPrice ??
        existingItem.companyPrice ??
        existingItem.partnerPrice ??
        retailPrice;
      const totalSavingsPerUnit = retailPrice - unitPrice;
  ...
  private saveEntireCart(companyId: string, items: B2BCartItem[]): void {
    ...
    if (items.length === 0) {
      ...
      if (currentCoupons.length > 0) {
        console.log(
          'B2B Cart is empty, clearing applied coupons for fresh session',
        );
        this.clearStoredCoupons(companyId);
  ...
  private calculateTieredPrice(
    ...
    product: any,
    basePrice: number,
  ): { unitPrice: number; appliedTier: 1 | 2 | 3 } {
    ...
    if (
      product.quantity_tier_3 &&
      product.price_tier_3 &&
      quantity >= product.quantity_tier_3
    ) {
    ...
    // Check tier 2
    if (
      product.quantity_tier_2 &&
      product.price_tier_2 &&
      quantity >= product.quantity_tier_2
    ) {
    ...
    // Check tier 1
    if (
      product.quantity_tier_1 &&
      product.price_tier_1 &&
      quantity >= product.quantity_tier_1
    ) {
  ...
  private recalculateTieredPricing(items: B2BCartItem[]): B2BCartItem[] {
    return items.map((item) => {
      ...
      const basePrice =
        item.originalUnitPrice || item.companyPrice || item.retailPrice;
      ...
      if (
        item.quantityTier3 &&
        item.priceTier3 &&
        item.quantity >= item.quantityTier3
      ) {
      ...
      // Check tier 2
        ...
        item.quantityTier2 &&
        item.priceTier2 &&
        item.quantity >= item.quantityTier2
      ) {
      ...
      // Check tier 1
        ...
        item.quantityTier1 &&
        item.priceTier1 &&
        item.quantity >= item.quantityTier1
      ) {
      ...
      const standardSavingsPerUnit = Math.max(
        0,
        retailPrice - (item.originalUnitPrice || basePrice),
      );
      const additionalSavingsPerUnit = Math.max(
        0,
        totalSavingsPerUnit - standardSavingsPerUnit,
      ...
      const updatedOriginalUnitPrice = item.isBundle
        ? unitPrice
   ...
   */
  private recalculateBundleStatus(
    items: B2BCartItem[],
    companyId: string,
  ): B2BCartItem[] {
    ...
    const updatedItems = items.map((item) => {
      ...
      if (bundleComplete && originalDiscount !== undefined) {
        ...
        newAdditionalSavings =
          Math.max(0, totalSavingsPerUnit - standardSavingsPerUnit) *
          item.quantity;
  ...
  private generateId(): string {
    return (
      'cart_item_' + Date.now() + '_' + Math.random().toString(36).substr(2, 9)
    );
  }
   ...
   */
  private async getProductWithPricing(
    productId: string,
    companyId: string,
  ): Promise<any> {
    ...
    try {
      ...
      console.warn(
        'Company pricing not available for product:',
        productId,
        error,
      );
  ...
  private parseStoredCartItem(item: any): B2BCartItem {
    return {
      ...
      partnerOfferAppliedAt: item.partnerOfferAppliedAt
        ? new Date(item.partnerOfferAppliedAt)
        : item.partnerOfferAppliedAt,
      ...
      quantityTier1: item.quantityTier1
        ? Number(item.quantityTier1)
        : undefined,
      priceTier2: item.priceTier2 ? Number(item.priceTier2) : undefined,
      quantityTier2: item.quantityTier2
        ? Number(item.quantityTier2)
        : undefined,
      priceTier3: item.priceTier3 ? Number(item.priceTier3) : undefined,
      quantityTier3: item.quantityTier3
        ? Number(item.quantityTier3)
        : undefined,
   ...
   */
  async getPartnerPricingDetails(
    productId: string,
    companyId: string,
  ): Promise<any | null> {
    try {
  ...
  private async applyCouponAsync(
    ...
    try {
      const mappedItems: CartItem[] = cartItems.map((item) =>
        this.mapB2BItemToCartItem(item),
      );
      const validationResult: CouponValidationResult = await firstValueFrom(
        this.couponValidationService.validateCoupon(code, mappedItems),
      );
      ...
      if (
        ...
        throw new Error(
          validationResult.errorMessage ||
            this.translationService.translate('cart.couponValidationError'),
        );
      }
      ...
      if (storedCoupons.length > 0) {
        throw new Error(
          this.translationService.translate('cart.singleCouponOnly'),
        );
      }

      const duplicateCoupon = storedCoupons.some(
        (coupon) =>
          coupon.code.toLowerCase() ===
          validationResult.coupon!.code.toLowerCase(),
      );
      if (duplicateCoupon) {
        throw new Error(
          this.translationService.translate('cart.couponAlreadyApplied'),
        );
      }

      // Check if this is a bundle offer coupon
      const isBundleOffer = await this.checkIfBundleOffer(
        validationResult.coupon.id,
      );
      if (isBundleOffer) {
        // Verify all products from the bundle are in the cart
        const hasAllBundleProducts = await this.verifyBundleProductsInCart(
          validationResult.coupon.id,
          cartItems,
        );
        if (!hasAllBundleProducts) {
          throw new Error(
            this.translationService.translate(
              'b2b.offers.bundleRequiresAllProductsError',
            ),
          );
        }
      }
      ...
      if (validationResult.coupon.id) {
        try {
          await firstValueFrom(
            this.couponValidationService.incrementOfferUsage(
              validationResult.coupon.id,
            ),
          );
        } catch (error) {
   ...
   */
  private async verifyBundleProductsInCart(
    offerId: string,
    cartItems: B2BCartItem[],
  ): Promise<boolean> {
    try {
  ...
  private mapB2BItemToCartItem(item: B2BCartItem): CartItem {
    const addedAt =
      item.addedAt instanceof Date ? item.addedAt : new Date(item.addedAt);
    const offerType = item.partnerOfferType;
    const mappedOfferType:
      | 'percentage'
      | 'fixed_amount'
      | 'buy_x_get_y'
      | 'bundle'
    ...
    return {
      ...
      offerAppliedAt: item.partnerOfferAppliedAt
        ? item.partnerOfferAppliedAt instanceof Date
          ? item.partnerOfferAppliedAt.toISOString()
          : item.partnerOfferAppliedAt
        : undefined,
  ...
  private saveStoredCoupons(
    companyId: string,
    coupons: B2BAppliedCoupon[],
  ): void {
    localStorage.setItem(
      this.getCouponStorageKey(companyId),
