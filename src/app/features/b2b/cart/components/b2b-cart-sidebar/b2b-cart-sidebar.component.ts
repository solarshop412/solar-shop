...
export class B2BCartSidebarComponent implements OnDestroy {
  ...
  constructor(
    ...
    this.cartSummary$ = this.store.select(
      B2BCartSelectors.selectB2BCartSummary,
    );
    ...
    this.companyInfo$ = this.store.select(
      B2BCartSelectors.selectB2BCartCompanyInfo,
    );
    this.sidebarOpen$ = this.store.select(
      B2BCartSelectors.selectB2BCartSidebarOpen,
    );
    this.appliedCoupons$ = this.store.select(
      B2BCartSelectors.selectB2BCartAppliedCoupons,
    );
    this.couponError$ = this.store.select(
      B2BCartSelectors.selectB2BCartCouponError,
    );
    this.isCouponLoading$ = this.store.select(
      B2BCartSelectors.selectB2BCartIsCouponLoading,
    );
    this.hasCouponsApplied$ = this.store.select(
      B2BCartSelectors.selectB2BCartHasCoupons,
    );
    this.hasMinimumOrderViolations$ = this.cartItems$.pipe(
      takeUntil(this.destroy$),
      // Check if any item has quantity below minimum order
      map((items: B2BCartItem[]) =>
        items.some((item) => item.quantity < item.minimumOrder),
      ),
    );
  }
  ...
  applyCoupon(): void {
    ...
    this.companyInfo$.pipe(take(1)).subscribe((info) => {
      ...
      this.store.dispatch(
        B2BCartActions.applyB2BCoupon({ code, companyId: info.companyId }),
      );
      this.couponCode = '';
    });
  }
  ...
  increaseQuantity(productId: string): void {
    ...
    this.cartItems$
      ...
      .subscribe((items: B2BCartItem[]) => {
        ...
        if (item) {
          this.store.dispatch(
            B2BCartActions.updateB2BCartItem({
              productId,
  ...
  getNextTierHint(item: B2BCartItem): string {
    ...
    if (item.appliedTier === 1 && item.quantityTier2 && item.priceTier2) {
      ...
      if (neededQty > 0 && neededQty <= 5) {
        const savings = (
          (item.unitPrice - item.priceTier2) *
          item.quantityTier2
        ).toFixed(2);
      ...
      if (neededQty > 0 && neededQty <= 10) {
        const savings = (
          (item.unitPrice - item.priceTier3) *
          item.quantityTier3
        ).toFixed(2);
        return this.translationService.translate('b2bCart.addMoreForTier', {
          qty: neededQty,
