...
export const b2bCartReducer = createReducer(
  ...
  on(
    ...
    (
      state,
      { items, companyId, companyName, appliedCoupons, couponDiscount },
    ) => ({
      ...state,
      items,
      companyId,
  ...
  on(B2BCartActions.removeB2BCouponSuccess, (state, { couponId }) => {
    ...
    return {
      ...
      couponDiscount: Math.max(
        0,
        Math.round((state.couponDiscount - discountToRemove) * 100) / 100,
      ),
      isCouponLoading: false,
      couponError: null,
    };
...
function calculateCartTotals(items: B2BCartItem[]) {
  ...
  const totalSavings = items.reduce(
    (sum, item) => sum + (item.retailPrice - item.unitPrice) * item.quantity,
    0,
  );
