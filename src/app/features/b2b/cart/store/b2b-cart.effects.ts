...
export class B2BCartEffects {
          ...
          catchError((error) =>
            of(
              B2BCartActions.loadB2BCartFailure({
                error:
                  error.message ||
                  this.translationService.translate('b2bCart.failedToLoad'),
              }),
            ),
          ),
        ),
          ...
          catchError((error) =>
            of(
              B2BCartActions.addToB2BCartFailure({
                error:
                  error.message ||
                  this.translationService.translate('b2bCart.failedToAdd'),
              }),
            ),
          ),
        ),
  ...
  updateB2BCartItem$ = createEffect(() =>
    this.actions$.pipe(
      ...
      withLatestFrom(
        this.store.select(B2BCartSelectors.selectB2BCartCompanyInfo),
      ),
          ...
          return of(
            B2BCartActions.updateB2BCartItemFailure({
              error: this.translationService.translate(
                'b2bCart.companyInfoNotAvailable',
              ),
            }),
          );
        }

        return this.b2bCartService
          .updateCartItemWithPricing(productId, quantity, companyInfo.companyId)
          .pipe(
            switchMap((updatedItem) => {
              if (updatedItem) {
                // Item was updated with new pricing
                return of(
                  B2BCartActions.updateB2BCartItemWithPricing({ updatedItem }),
                );
              } else {
                // Item was removed (quantity = 0), dispatch success action
                return of(
                  B2BCartActions.updateB2BCartItemSuccess({
                    productId,
                    quantity: 0,
                  }),
                );
            ...
            catchError((error) =>
              of(
                B2BCartActions.updateB2BCartItemFailure({
                  error:
                    error.message ||
                    this.translationService.translate('b2bCart.failedToUpdate'),
                }),
              ),
            ),
          );
  ...
  removeFromB2BCart$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap(([{ productId }, companyInfo]) => {
        ...
        return this.b2bCartService
          .removeFromCart(productId, companyInfo.companyId)
          .pipe(
            switchMap(() =>
            ...
            catchError((error) =>
              of(
                B2BCartActions.removeFromB2BCartFailure({
                  error:
                    error.message ||
                    this.translationService.translate('b2bCart.failedToRemove'),
                }),
              ),
            ),
          );
          ...
          catchError((error) =>
            of(
              B2BCartActions.clearB2BCartFailure({
                error:
                  error.message ||
                  this.translationService.translate('b2bCart.failedToClear'),
              }),
            ),
          ),
        );
          ...
          catchError((error) =>
            of(
              B2BCartActions.syncB2BCartFailure({
                error:
                  error.message ||
                  this.translationService.translate('b2bCart.failedToSync'),
              }),
            ),
          ),
        ),
  ...
  addToCartSuccess$ = createEffect(
    () =>
      this.actions$.pipe(
        ...
        tap(({ item }) => {
          const addedToCartMessage = this.translationService.translate(
            'b2bCart.addedToCart',
          );
          this.toastService.showSuccess(`${item.name} ${addedToCartMessage}`);
  ...
  applyCouponSuccess$ = createEffect(
    () =>
      this.actions$.pipe(
        ...
        tap(({ coupon }) => {
          this.toastService.showSuccess(
            this.translationService.translate('cart.couponAppliedSuccess', {
              code: coupon.code,
            }),
          );
        }),
    ...
    this.actions$.pipe(
      ...
      switchMap(([{ couponId }, companyId, appliedCoupons]) => {
        ...
        return this.b2bCartService.removeCoupon(couponId, companyId).pipe(
          map(() =>
            B2BCartActions.removeB2BCouponSuccess({
              couponId,
      ...
      this.actions$.pipe(
        ...
        tap(({ couponCode }) => {
          const message = couponCode
            ? this.translationService.translate('cart.couponRemoved', {
                code: couponCode,
              })
    ...
    this.actions$.pipe(
      ...
      switchMap(
        ({
          ...
          const addObservables = products.map(
            ({
              productId,
              quantity,
              individualDiscount,
              ...
              this.b2bCartService
                .addToCart(productId, quantity, companyId, {
                  partnerOfferId,
                  partnerOfferName,
                  partnerOfferType,
                  partnerOfferDiscount,
          ...
          return forkJoin(addObservables).pipe(
            switchMap((results) => {
              ...
              return this.b2bCartService.loadCart(companyId).pipe(
                map(
                  ({ items, companyName, appliedCoupons, couponDiscount }) => {
        ...
        tap(({ addedCount, skippedCount }) => {
          if (addedCount > 0) {
            const message = this.translationService.translate(
              'b2bCart.productsAddedToCart',
              { count: addedCount },
            );
            this.toastService.showSuccess(message);

            if (skippedCount > 0) {
              const warningMessage = this.translationService.translate(
                'b2bCart.productsSkipped',
                { count: skippedCount },
              );
            ...
            this.toastService.showError(
              this.translationService.translate('b2bCart.noProductsAdded'),
            );
          }
        }),
      ),
