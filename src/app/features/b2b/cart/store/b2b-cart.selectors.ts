...
export const selectB2BCartState =
  createFeatureSelector<B2BCartState>('b2bCart');
...
export const selectB2BCartItemById = (productId: string) =>
  createSelector(selectB2BCartItems, (items) =>
    items.find((item) => item.productId === productId),
  );

export const selectB2BCartItemQuantity = (productId: string) =>
  createSelector(selectB2BCartItems, (items) => {
  ...
  (items) => {
    return items.map((item) => ({
      ...
      savingsPercentage: Math.round(
        ((item.retailPrice - item.unitPrice) / item.retailPrice) * 100,
      ),
    }));
  },
);
...
export const selectIsProductInB2BCart = (productId: string) =>
  createSelector(selectB2BCartItems, (items) =>
    items.some((item) => item.productId === productId),
  );

// Get product pricing info for cart context
export const selectB2BCartProductPricing = (productId: string) =>
  createSelector(selectB2BCartItems, (items) => {
    const item = items.find((item) => item.productId === productId);
    if (!item) return null;
...
export const selectB2BCartCouponById = (couponId: string) =>
  createSelector(selectB2BCartAppliedCoupons, (coupons) =>
