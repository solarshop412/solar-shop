...
export class CouponValidationService {
  ...
  private async validateCouponAsync(
    code: string,
    cartItems: CartItem[],
    ...
    try {
      ...
      const offerValidation = await this.validateOfferBasedRules(
        coupon,
        cartItems,
      );
      ...
      const productValidation = this.validateProductAndCategoryRules(
        coupon,
        cartItems,
      );
  ...
  private async getCouponByCode(code: string): Promise<Coupon | null> {
    try {
      ...
      const { data, error } = await this.supabaseService.client
        ...
        .select('*')
        .or(
          `code.eq."${code}",code.eq."${code.toUpperCase()}",code.eq."${code.toLowerCase()}"`,
        )
        .not('code', 'is', null);
      ...
      const coupon = {
        ...
        maxUsage:
          couponData.max_usage !== null && couponData.max_usage !== undefined
            ? couponData.max_usage
  ...
  private validateBasicCouponRules(
    ...
    if (this.usedCouponsInSession.has(coupon.code.toLowerCase())) {
      console.log(
        'Coupon already applied - allowing reapplication for new products:',
        coupon.code,
      );
    }
  ...
  private async validateOfferBasedRules(
    coupon: Coupon,
    cartItems: CartItem[],
  ): Promise<CouponValidationResult> {
    ...
    const itemsWithOtherOffers = cartItems.filter((item) => {
      ...
      const isSameCoupon =
        item.offerName &&
        item.offerName.toLowerCase().includes(coupon.code.toLowerCase());
    ...
    if (itemsWithOtherOffers.length > 0) {
      console.log(
        'Cart contains items with DIFFERENT offer discounts:',
        itemsWithOtherOffers.map((item) => ({
          name: item.name,
      ...
      return {
        ...
        errorMessage:
          this.translationService.translate(
            'cart.couponCannotApplyToDiscountedProducts',
          ) ||
          'Cannot apply coupon to products that already have offer discounts',
        discountAmount: undefined,
        coupon: undefined,
      };
    }
      ...
      if (coupon.applicableOfferIds?.length) {
        ...
        if (!hasApplicableOffer) {
          return {
            isValid: false,
            errorMessage: this.translationService.translate(
              'cart.couponNotApplicableToOffers',
            ),
            discountAmount: undefined,
            coupon: undefined,
          };
  ...
  private validateProductAndCategoryRules(
    coupon: Coupon,
    cartItems: CartItem[],
  ): CouponValidationResult {
    ...
    if (coupon.applicableProductIds?.length) {
      ...
      if (!hasApplicableProduct) {
        return {
          isValid: false,
          errorMessage: this.translationService.translate(
            'cart.couponNotApplicableToProducts',
          ),
          discountAmount: undefined,
          coupon: undefined,
        };
    ...
    if (coupon.excludedProductIds?.length) {
      ...
      if (hasExcludedProduct) {
        return {
          isValid: false,
          errorMessage: this.translationService.translate(
            'cart.couponExcludedFromProducts',
          ),
          discountAmount: undefined,
          coupon: undefined,
        };
    ...
    if (coupon.applicableCategories?.length) {
      ...
      if (!hasApplicableCategory) {
        return {
          isValid: false,
          errorMessage: this.translationService.translate(
            'cart.couponNotApplicableToCategories',
          ),
          discountAmount: undefined,
          coupon: undefined,
        };
    ...
    if (coupon.excludedCategories?.length) {
      ...
      if (hasExcludedCategory) {
        return {
          isValid: false,
          errorMessage: this.translationService.translate(
            'cart.couponExcludedFromCategories',
          ),
        };
      }
    }
  ...
  private async calculateDiscountAmount(
    ...
    const individualDiscountAmount =
      await this.calculateIndividualProductDiscounts(coupon.id, cartItems);
    if (individualDiscountAmount > 0) {
      console.log(
        'Applied individual product discounts:',
    ...
    console.log(
      'No individual product discounts found, applying general discount to eligible items',
    );
    ...
    switch (coupon.discountType) {
      case 'percentage':
        ...
        console.log(
          `Percentage discount: ${coupon.discountValue}% of €${eligibleTotal} = €${discountAmount}`,
        );
        break;
      case 'fixed_amount':
        discountAmount = Math.min(coupon.discountValue, eligibleTotal);
        console.log(
          `Fixed amount discount: €${coupon.discountValue} (limited to total €${eligibleTotal}) = €${discountAmount}`,
        );
        break;
      case 'free_shipping':
    ...
    if (coupon.maxDiscountAmount && discountAmount > coupon.maxDiscountAmount) {
      console.log(
        `Discount capped at maximum: €${coupon.maxDiscountAmount} (was €${discountAmount})`,
      );
  ...
  private getEligibleItems(coupon: Coupon, cartItems: CartItem[]): CartItem[] {
    ...
    const eligibleItems = cartItems.filter((item) => {
      ...
      if (
        ...
        console.log(
          `Item ${item.name} excluded: category ${item.category} not in applicable categories`,
        );
        return false;
      }
      if (
        ...
        console.log(
          `Item ${item.name} excluded: category ${item.category} in excluded categories`,
        );
        return false;
      }
      ...
      if (coupon.applicableOfferIds?.length) {
        if (
          !item.offerId ||
          !coupon.applicableOfferIds.includes(item.offerId)
        ) {
      ...
      if (
        coupon.excludedOfferIds?.length &&
        item.offerId &&
        coupon.excludedOfferIds.includes(item.offerId)
      ) {
    ...
    console.log(
      `Found ${eligibleItems.length} eligible items for general discount out of ${cartItems.length} total items`,
    );
   ...
   */
  private async calculateIndividualProductDiscounts(
    couponId: string,
    cartItems: CartItem[],
  ): Promise<number> {
    try {
      ...
      if (!offerProducts || offerProducts.length === 0) {
        console.log(
          '✅ No specific product discounts found - will use category/general discount',
        );
        return 0;
      }
      ...
      console.log(
        '📋 Available product discounts in database:',
      ...
      console.log(
        '🔍 Checking which cart items match specific product discounts...',
      );

      for (const cartItem of cartItems) {
        const productDiscount = offerProducts.find(
          (op) => op.product_id === cartItem.productId,
        );
        ...
        if (productDiscount) {
          ...
          console.log(
            `✅ MATCH FOUND: ${cartItem.name} (ID: ${cartItem.productId})`,
          );
          ...
          const discountAmount =
            productDiscount.discount_amount || productDiscount.amount || 0;
          ...
          if (discountType === 'fixed_amount' && discountAmount > 0) {
            ...
            console.log(
              `   💰 Fixed discount: €${discountAmount} × ${cartItem.quantity} units = €${itemDiscount}`,
            );
            ...
            console.log(
              `   📊 Percentage discount: ${discountPercentage}% of €${itemTotal} = €${itemDiscount}`,
            );
          } else {
            console.log(
              `   ❌ No valid discount found: type=${discountType}, amount=${discountAmount}, percentage=${discountPercentage}`,
            );
          }

          totalDiscount += itemDiscount;
          console.log(
            `   📈 Running total after ${cartItem.name}: €${totalDiscount}`,
          );
        } else {
          console.log(
            `❌ NO MATCH: ${cartItem.name} (ID: ${cartItem.productId}) - not in specific product discounts`,
          );
        }
      }
      ...
      if (hasMatchingProducts) {
        ...
        console.log(
          `   Applied to ${matchingCount} out of ${cartItems.length} cart items`,
        );
        ...
        console.log(
          '❌ No cart items match ANY specific products - will use category/general discount',
        );
        console.log('=== END INDIVIDUAL PRODUCT CALCULATION ===');
        return 0;
      }
  ...
  private async getAvailableCouponsAsync(
    cartItems: CartItem[],
  ): Promise<Coupon[]> {
    try {
      ...
      const coupons: Coupon[] = data.map((item) => ({
        ...
        maxUsage:
          item.max_usage !== null && item.max_usage !== undefined
            ? item.max_usage
      ...
      return coupons.filter((coupon) =>
        this.isPotentiallyApplicable(coupon, cartItems),
      );
    } catch (error) {
      console.error('Error fetching available coupons:', error);
      return [];
  ...
  private isPotentiallyApplicable(
    coupon: Coupon,
    cartItems: CartItem[],
  ): boolean {
  ...
  private async incrementOfferUsageAsync(offerId: string): Promise<boolean> {
    try {
      ...
      const { data: offerData, error: fetchError } =
      ...
      console.log('Successfully used coupon:', {
        ...
        newUsagesLeft:
          usagesLeft !== null && usagesLeft !== undefined
            ? Number(usagesLeft) - 1
            : 'unlimited',
      });
