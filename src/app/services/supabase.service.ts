...
export class SupabaseService {
  ...
  async updateRecord<T extends keyof Database['public']['Tables']>(
    ...
    if (error) {
      ...
      throw new Error(
        `Failed to update ${tableName}: ${error.message} (Code: ${error.code})`,
      );
    }

    if (!data) {
      console.error(
        `No data returned after updating ${tableName} with ID: ${id}`,
      );
      throw new Error(
        `Update operation completed but no data was returned for ${tableName} with ID: ${id}`,
      );
    }
  ...
  async adminDeleteRecord<T extends keyof Database['public']['Tables']>(
    ...
    console.log(
      `Admin attempting to delete ${tableName} record with id: ${id}`,
    );
    ...
    if (fetchError) {
      console.error(
        `Error fetching ${tableName} record for deletion:`,
        fetchError,
      );
      throw new Error(
        `Failed to find ${tableName} record: ${fetchError.message}`,
      );
    }
    ...
    if (error) {
      ...
      throw new Error(
        `Failed to delete ${tableName}: ${error.message} (Code: ${error.code})`,
      );
    }
  ...
  async adminUpdateRecord<T extends keyof Database['public']['Tables']>(
    ...
    console.log(
      `Admin attempting to update ${tableName} record with id: ${id}`,
      record,
    );
    ...
    if (fetchError) {
      console.error(
        `Error fetching ${tableName} record for update:`,
        fetchError,
      );
      throw new Error(
        `Failed to find ${tableName} record: ${fetchError.message}`,
  ...
  async getProducts(filters?: {
    ...
    if (filters?.categoryId) {
      ...
      query = query.or(
        `category_id.eq.${filters.categoryId},product_categories.category_id.eq.${filters.categoryId}`,
      );
    }
  ...
  async getCartItems(userId?: string, sessionId?: string) {
    let query = this.supabase.from('cart_items').select(`
                *,
                products:product_id (
                    id,
                    name,
  ...
  async getBlogPosts(filters?: {
    ...
    const { data: posts, error } = await query.order('published_at', {
      ascending: false,
    });
    ...
    const postsWithRelations = posts.map((post) => ({
      ...
      categories:
        categories.find((category) => category.id === post.category_id) || null,
    }));
  ...
  async incrementBlogPostViews(id: string) {
    const { error } = await this.supabase.rpc('increment_blog_post_views', {
      post_id: id,
    });
    if (error) throw error;
  }
  ...
  async findAuthUserByEmail(
    email: string,
  ): Promise<{ id: string; email: string; profile?: any } | null> {
    try {
      const { data, error } = await this.supabase.rpc('find_user_by_email', {
        user_email: email,
      });
  ...
  async decrementProductStock(
    ...
    try {
      ...
      console.log(
        `Stock updated successfully for product ${productId}: ${currentStock} -> ${newStock}`,
      );
      return true;
    } catch (error) {
  ...
  async processOrderStockAdjustment(
    orderItems: any[],
    decrement: boolean = true,
  ): Promise<boolean> {
    try {
      console.log(
        `Processing stock adjustment for ${orderItems.length} items (decrement: ${decrement})`,
      );
      ...
      for (const item of orderItems) {
        ...
        if (!productId) {
          console.log(
            `Skipping stock adjustment for item without product ID:`,
            item,
          );
        ...
        console.log(
          `Processing item with productId: ${productId}, quantity: ${item.quantity}`,
        );
